...
*/
/**************************************************************************
Implementar un contador binario que incrementara cada vez que se presione un boton,
este sera configurado como entrada digital. Para visualizar el conteo se utilizan 8 LEDs.
**************************************************************************/
...
void main(void)
{
  SisInit ();
   ...
   while (1) //Mientras sea verdadero se ejecuta el siguiente codigo
   {
      if (!input (Boton)) //Pregunta el estado del boton mediante una comparacion
      {
         cont++; //Se incrementa el contador
         output_b (cont); //Se asigna el valor de cont al puerto B y se despliega el valor
         Delay_ms (5); //Retardo
         while (!input (Boton)); //Mientras el boton se mantenga igual a cero
         Delay_ms (5);  //Retardo
      }
   }
}
