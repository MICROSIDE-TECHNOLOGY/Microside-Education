...
*/
/**************************************************************************
Esta practica consiste en realizar una medicion de distancia a traves de un 
...
#define trig pin_C1     // Conectar al entrenador el pin "TRIGGER" del sensor ultrasonico
#define echo pin_C0     // Conectar al entrenador el pin "ECHO" del sensor ultrasonico
...
void main()
{
   ...
   while (1)
   {
      output_high (trig); // Envia pulso alto al pin TRIGGER del sensor
      delay_us (100);  // Mantiene pulso 100uS.
      output_low (trig); // Desactiva pulso
      delay_us (350);
      while ( ! input (echo)){} // Espera por estado alto del pin ECHO del sensor ultrasonico
      set_timer1 (0); // Ajusta el Timer1 a 0
      while (input (echo)){} // Espera por estado bajo del pin ECHO del sensor ultrasonico
